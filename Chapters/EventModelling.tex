...
\begin{itemize}
	\item Current approaches to modelling events using ontologies
	\item Our approach to modelling interaction events, based on the concepts of the existing event ontologies
	\item Categorising interaction events in terms of the layers of interaction described above %, as well as in terms of feedback
\end{itemize}
...
In the iStuff toolkit \cite{Ballagas2003} \emph{hierarchical event} structures were used to abstract low-level events into application-level events. \marginpar{Also see the related work on task models in Section \ref{interactionTasks}.} We also introduce the notion of an event hierarchy, where the designer of the smart object can specify at which level the event occurs. %where low-level events are considered to be very \emph{generic}, as they do not report a user's intention directly \cite{Niezen2011}. %These low-level events first need to be transformed into intentional events---events that express user intention.
